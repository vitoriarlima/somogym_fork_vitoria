...
class PlanarReaching(SomoEnv.SomoEnv):
    def __init__(
        ...
        if vary_n_segs:
            n_seg_new = run_config["n_segments"]
        ...
        super().__init__(
            ...
            ground_plane_height=None,
            orig_manipulator_def_dict=manipulator_def_dict,
            run_ID=run_ID,
            render=render,
            debug=debug,
        )
    ...
    def get_observation(self):
        ...
        if "tip_orientation_error" in obs_flags:
            ...
            n_segments = len(self.manipulators[0].flexible_joint_indices)
            last_link_state = p.getLinkState(
                self.manipulators[0].bodyUniqueId,
                n_segments - 1,
                physicsClientId=self.physics_client,
