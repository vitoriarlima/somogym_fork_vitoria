...
class SomoEnv(gym.Env):
    def __init__(
        ...
        self.orig_manipulator_def_dict = orig_manipulator_def_dict
    ...
    def update_n_segments(self, new_n_segments):

        manipulator_def_dict = vary_segment_number.vary_segment_number(
            self.orig_manipulator_def_dict, new_n_segments
        )
        ...
        manipulator_def.urdf_filename = os.path.join(
            ...
            "PlanarBlockPushing/" +
            "definitions/" + manipulator_def.manipulator_name + f"{new_n_segments}segs.urdf",
        )

        self.manipulators = [SMContinuumManipulator(manipulator_def)]

        return True

    def reset(self, run_render=False, n_segments = None):

        # modified reset
        # n_segments = int(self.ep_count/1.)+1
        # n_segments = 1
        #
        # if n_segments>10:
        #     n_segments=10
        if n_segments is not None:
            self.update_n_segments(new_n_segments=n_segments)
            # this new n_segments should be overwriting the one in the config file
            # self.run_config['n_segments'] = n_segments
